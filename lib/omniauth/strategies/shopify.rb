  ...
  module Strategies
    class Shopify < OmniAuth::Strategies::OAuth2
      ...
      option :setup, proc { |env|
        ...
        shopify_auth_params &&= shopify_auth_params.with_indifferent_access
      ...
      def valid_site?
        !!(%r{\A(https|http)://[a-zA-Z0-9][a-zA-Z0-9\-]*\.#{Regexp.quote(options[:myshopify_domain])}/?\z} =~ options[:client_options][:site])
      end

      def valid_signature?
        print "request.POST.empty? #{request.POST.empty?}\n"
        ...
        time_test = timestamp.to_i > Time.now.to_i - CODE_EXPIRES_AFTER
        print "timestamp.to_i > Time.now.to_i - CODE_EXPIRES_AFTER #{time_test}\n"
        print "timestamp #{timestamp}\n"
        print "timestamp.to_i #{timestamp.to_i}\n"
        print "Time.now.to_i #{Time.now.to_i}\n"
        print "Time.now #{Time.now}\n"
        print "CODE_EXPIRES_AFTER #{CODE_EXPIRES_AFTER}\n"
      ...
      def normalized_scopes(scopes)
        ...
        ignore_scopes = scope_list.map do |scope|
          scope =~ /\A(unauthenticated_)?write_(.*)\z/ && "#{Regexp.last_match(1)}read_#{Regexp.last_match(2)}"
        end.compact
      ...
      def self.hmac_sign(encoded_params, secret)
        OpenSSL::HMAC.hexdigest(OpenSSL::Digest.new('SHA256'), secret, encoded_params)
      end
      ...
      def callback_phase
        unless valid_site?
          return fail!(:invalid_site,
                       CallbackError.new(:invalid_site, 'OAuth endpoint is not a myshopify site.'))
        end
        unless valid_signature?
          return fail!(:invalid_signature,
                       CallbackError.new(:invalid_signature,
                                         'Signature does not match, it may have been tampered with.'))
        end

        token = build_access_token
        unless valid_scope?(token)
          return fail!(:invalid_scope,
                       CallbackError.new(:invalid_scope, 'Scope does not match, it may have been tampered with.'))
        end
        unless valid_permissions?(token)
          return fail!(:invalid_permissions,
                       CallbackError.new(:invalid_permissions, 'Requested API access mode does not match.'))
        end
